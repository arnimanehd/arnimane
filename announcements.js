...
function announcementCard(item, featured = false) {
  ...
  return `
    <article class="announcement-card ${featured ? "featured" : ""}">
      ${image}
...
async function loadHomepageAnnouncements() {
  const featuredContainer =
    document.getElementById("featuredAnnouncement");

  const announcementStrip =
    document.getElementById("announcementStrip");

  if (!featuredContainer || !announcementStrip) {
    return;
  }
  ...
  try {
      ...
      featuredContainer.innerHTML = `
        <article class="announcement-card featured">
          <div class="announcement-content">
            ...
            <p>
              Your first published Sanity announcement will
    ...
    const featuredAnnouncement =
      announcements.find(item => item.featured) ||
      announcements[0];
